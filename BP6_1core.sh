...
if [[ $# -ne 6 ]]; then
	...
	echo "example:" $0 "./stockfish_base ./stockfish_test 5 11 depth pure"
    exit 1
fi
...
($base bench 16 1 10 default depth nnue >/dev/null 2>&1)&
...
for k in `seq 1 $n_runs`;
  do
    ...
    if [ $((k%2)) -eq 0 ];
	  then
        ($base bench 16 1 $num default $type $NNtype >/dev/null 2>> base000.txt)&
        wait
        ($test bench 16 1 $num default $type $NNtype >/dev/null 2>> test000.txt)&
        wait
      else
        ($test bench 16 1 $num default $type $NNtype >/dev/null 2>> test000.txt)&
        wait
...
cat tmp000.txt | awk '{sum1 += $1 ; sumq1 += $1**2 ;sum2 += $2 ; sumq2 += $2**2 ;sum3 += $3 ; sumq3 += $3**2 } END {printf "\nbase = %10d +/- %d\ntest = %10d +/- %d\ndiff = %10d +/- %d\nSchnellerInProzent = %.6f\n\n", sum1/NR , NR/(NR-1)*sqrt(sumq1/NR - (sum1/NR)**2)/sqrt(NR)*1.96 , sum2/NR , NR/(NR-1)*sqrt(sumq2/NR - (sum2/NR)**2)/sqrt(NR)*1.96 , sum3/NR  , NR/(NR-1)*sqrt(sumq3/NR - (sum3/NR)**2)/sqrt(NR)*1.96 , 100 * (sum2 - sum1)/sum1 }'
